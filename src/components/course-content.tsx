...
function RichText({ text, definitions, onDefinitionPress, className }:{text:string;definitions:Definition[];onDefinitionPress?:(definition:Definition)=>void;className:string}) {
  ...
  while((match=matcher.exec(value))){const start=match.index,end=start+match[0].length;
    ...
    parts.push(<Text key={`${start}-${match[0]}`} accessibilityRole="button" accessibilityHint="Affiche la définition" onPress={()=>definition&&onDefinitionPress?.(definition)} style={{textDecorationLine:'underline',textDecorationStyle:'dotted'}} className="font-extrabold text-primary">{match[0]}</Text>);
    cursor=end;
  }
...
export function CourseContent({ markdown, definitions=[], onDefinitionPress, onImagePress, activeRange, listening=false, onBlockLayout }: { markdown: string;definitions?:Definition[];onDefinitionPress?:(definition:Definition)=>void;onImagePress?:(image:NonNullable<ViewerImage>)=>void;activeRange?:{startLine:number;endLine:number}|null;listening?:boolean;onBlockLayout?:(line:number,y:number)=>void }) {
  ...
  for(let lineIndex=0;lineIndex<lines.length;lineIndex++){const raw=lines[lineIndex];const sourceLine=lineIndex+1;const line=raw.trim(); if(!line){flush();continue;}
    ...
    if(line.startsWith('### ')){flush();push(<Text className="mb-3 mt-5 text-xl font-black text-ink">{clean(line.slice(4))}</Text>,sourceLine);continue;}
    if(line.startsWith('## ')){flush();push(<Text className="mb-4 mt-7 text-2xl font-black text-primary">{clean(line.slice(3))}</Text>,sourceLine);continue;}
