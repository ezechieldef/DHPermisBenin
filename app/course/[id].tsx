...
export default function CourseDetail() {
  ...
  return <>
    {process.env.EXPO_OS === 'web' ? null : <NativeKeepAwake />}
    <View className="flex-1 bg-background">
      <View className="border-b border-border bg-surface px-4 pb-3 pt-2">
        <View className="flex-row items-center">
          ...
          {subjectCount > 0 ? <Pressable accessibilityLabel="Voir les sujets du cours" onPress={() => router.push(`/course-subjects/${course.id}` as Href)} className="ml-2 h-11 w-11 items-center justify-center rounded-2xl bg-secondarySoft"><Ionicons name="school-outline" size={22} color={themeColors.secondary} /></Pressable> : null}
      ...
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 20, paddingTop: 22, paddingBottom: Math.max(28, insets.bottom + 18) }}>
        {currentStep.type === 'cover' ? <PartCover partNumber={structure.parts.findIndex((part) => part.key === currentStep.partKey) + 1} title={currentStep.partTitle} sectionCount={currentPart?.sectionStepKeys.length ?? 0} completedCount={currentPart?.sectionStepKeys.filter((key) => completed.has(key)).length ?? 0} onContinue={() => move(1)} /> : <>
          ...
          <CourseContent markdown={currentStep.markdown} lineOffset={currentStep.startLine - 1} definitions={definitions} onDefinitionPress={setSelected} />
          <View className="mt-8">
...
function NativeKeepAwake() {
  useKeepAwake('course-reading', { suppressDeactivateWarnings: true });
  return null;
}
