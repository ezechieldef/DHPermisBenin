...
export default function ExamScreen() {
  ...
  const start = async () => { const questions = await getExamQuestions(db, selectedPermitTypes); if (questions.length < 20) return Alert.alert('Questions insuffisantes', 'Sélectionnez davantage de types de permis dans les réglages.'); quiz.start({ mode:'exam', categoryId:null, categoryName:'Simulation d’examen', subjectIndex:null, questions, answers:{}, startedAt:Date.now() }); router.push('/quiz'); };
  return <Screen><Heading eyebrow="Mise en situation" title="Simulation d’examen" subtitle="20 questions équilibrées pour mesurer votre niveau dans les conditions d’un test." />
    ...
    <Text className="mt-4 text-center text-sm leading-5 text-inkMuted">Les questions correspondent aux types de permis sélectionnés dans vos réglages.</Text>
    <BrandFooter />
  </Screen>;
}
