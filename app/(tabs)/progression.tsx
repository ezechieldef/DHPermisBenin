...
export default function ProgressScreen() {
  const db=useSQLiteContext(); const [stats,setStats]=useState<DashboardStats|null>(null); const [history,setHistory]=useState<AttemptSummary[]>([]);
  useFocusEffect(useCallback(()=>{Promise.all([getDashboardStats(db),getRecentAttempts(db,30)]).then(([s,h])=>{setStats(s);setHistory(h);});},[db]));
  if (stats && !stats.attempts) return <Screen><Heading title="Votre progression" subtitle="Vos résultats apparaîtront ici après votre premier sujet."/><EmptyState icon="stats-chart" title="Votre parcours commence ici" message="Terminez un sujet ou une simulation pour obtenir vos premières statistiques."/></Screen>;
  return <Screen><Heading eyebrow="Historique local" title="Votre progression" subtitle="Suivez vos résultats et concentrez-vous sur les notions qui demandent encore de l’attention."/>
...
function Stat({label,value,icon}:{label:string;value:string;icon:keyof typeof Ionicons.glyphMap}){return <Card className="flex-1"><Ionicons name={icon} size={25} color={colors.primary}/><Text className="mt-3 text-3xl font-black text-ink">{value}</Text><Text className="text-sm text-inkMuted">{label}</Text></Card>}
